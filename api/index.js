...
const CONNECTION_URL = process.env.MONGODB_URI; // Use environment variable
const DATABASENAME = "LeagueOfLegends";
let database;
...
// Define routes regardless of the connection status
// ------Getters------
app.get('/api/LeagueOfLegends/users', async (req, res) => {
  userModel.find()
    .then(users => res.json(users))
...
// Avoid duplicate routes and correct any typos
app.get('/api/LeagueOfLegends/leaderboard', async (req, res) => {
  try {
    const result = await database.collection("leaderboard").find({}).toArray();
    res.send(result);
